...
#' \itemize{
#' \item The first columns contains a string of time units in chronological order.
...
# define default settings -------------------------------
  if (is.na(mintime))  { mintime <- dataset[1,1] }
  if (is.na(maxtime))  { maxtime <- dataset[nrow(dataset),1] }
  if (length(columns) == 1)  { columns <- 2:ncol(dataset) }

#  subset data for defined years -------------------------
  mdat <- dataset[which(dataset[,1] == mintime):which(dataset[,1] == maxtime), ]
  yrs <-  dataset[which(dataset[,1] == mintime):which(dataset[,1] == maxtime), 1]
